    ...
    if (intersects.length > 0) {
      ...
      if (currentTexture.img) {
        ...
        if (picName !== "crossed-out.png") {
          let texture1 = texturesLoaded.filter(
            (item) => item.image.currentSrc.split("/").pop() === picName
          );

          intersects[0].object.material[
            faceIndex
          ] = new THREE.MeshBasicMaterial({ map: texture1[0] });
        ...
        calculator.materialsArea.areas = calculator.getMaterialsArea(
          materials,
          cube
        );
